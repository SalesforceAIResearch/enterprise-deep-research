...
from services.activity_generator import ActivityManager  # Import the new ActivityManager class instead of ActivityGenerator
...
class EventBuffer:
    ...
    def check_heartbeat(self, current_timestamp=None):
        ...
        current_time = time.time()
        if current_time - self.last_heartbeat_time > 5.0:  # Send heartbeat every 5 seconds
    ...
    def process_event(self, event, time_now=None):
        ...
        if event_type == "node_start":
            ...
            if node_name == "generate_report":
                ...
                logger.info(f"Report received - length: {len(self.report_content) if self.report_content else 0} chars")
                
            if node_name == "reflect_on_report":
                reflections = event.get("reflections", None)
                needs_improvement = reflections and "needs_improvement" in reflections and reflections["needs_improvement"]
                self.report_needs_improvement = needs_improvement
                logger.info(f"Report reflection - needs improvement: {needs_improvement}")
                
            if node_name == "finalize_report":
                self.report_content = event.get("report", self.report_content)
                logger.info(f"Report finalized - length: {len(self.report_content) if self.report_content else 0} chars")
            ...
            if activity:
                ...
                if enriched_data:
                    ...
                    if 'domains' in enriched_data:
                        logger.info(f"Domain count: {len(enriched_data.get('domains', []))}")
                    if 'code_snippets' in enriched_data:
                        logger.info(f"Code snippets found: {len(enriched_data.get('code_snippets', []))} snippets")
    ...
    def _clean_content_for_frontend(self, content):
        ...
        logger.info(f"Starting cleaning for content (first 150 chars): {content[:150]}...")
    ...
    def _clean_all_string_content(self, event):
        ...
        if isinstance(event, dict) and event.get("event_type") == "activity_generated":
            data = event.get("data", {})
            if data.get("related_event_type") == "visualization_generated" and data.get("node_name") == "visualization":
                ...
                for i, img in enumerate(images):
                    ...
                    src_type = "base64" if has_src and img['src'].startswith('data:image') else "file_path" if has_src else "none"
                    src_length = len(img['src']) if has_src else 0
                    logger.info(f"  - Direct Viz Image {i+1} has src: {has_src} (type: {src_type}, length: {src_length})")
                code_snippets = data.get("enriched_data", {}).get("code_snippets", [])
                logger.info(f"Visualization activity has {len(code_snippets)} code snippets")
                for i, snippet in enumerate(code_snippets):
                    filename = snippet.get('filename', 'unknown')
                    logger.info(f"  - Direct Viz Code Snippet {i+1} has filename: {filename}")
                ...
                if event.get("data") and isinstance(event["data"], dict):
                    ...
                    for field in ["content", "report", "message", "running_summary", "activity"]:
                        if field in data and isinstance(data[field], str):
                            ...
                            if not has_code_snippets and ((content.strip().startswith('{') and content.strip().endswith('}')) or \
...
class ResearchService:
                ...
                if not attr.startswith('_'):
                    try:
                        ...
                        if not callable(value):
                            result[attr] = ResearchService._make_json_serializable(value)
    ...
    @staticmethod
    async def conduct_research(query: str, extra_effort: bool = False, minimum_effort: bool = False, benchmark_mode: bool = False, streaming: bool = False, stream_id: str = None, queue: Optional[Queue] = None, provider: Optional[str] = None, model: Optional[str] = None, uploaded_data_content: Optional[str] = None, uploaded_files: Optional[List[str]] = None):
        """Conduct research on a given query, putting events onto a queue if provided."""
        if streaming and not queue:
            logger.error(f"Streaming research requested for {stream_id} but no queue provided.")
            # Cannot proceed without a queue for streaming
            return 
            
        event_buffer = EventBuffer() # Create buffer for processing events before queuing
        ...
        if uploaded_data_content:
            ...
            logger.info(f"[UPLOAD_TRACE] Content length: {len(uploaded_data_content)} characters")
            logger.info(f"[UPLOAD_TRACE] Content preview (first 100 chars): {uploaded_data_content[:100]}...")
            logger.info(f"[UPLOAD_TRACE] Content type: {type(uploaded_data_content)}")
        else:
            logger.info(f"[UPLOAD_TRACE] No uploaded data content in service (value: {uploaded_data_content})")
        
        if uploaded_files:
            logger.info(f"[UPLOAD_TRACE] Uploaded files received: {len(uploaded_files)} file(s)")
        ...
        logger.info(f"Extra effort: {extra_effort}, Minimum effort: {minimum_effort}, Benchmark mode: {benchmark_mode}")
        logger.info(f"Activity filtering enabled: {activity_config['enable_activity_filtering']}")
        logger.info(f"Provider: {provider or os.environ.get('LLM_PROVIDER', 'openai')}, Model: {model or os.environ.get('LLM_MODEL', 'o3-mini')}")
        ...
        try:
            ...
            with_extra_config = os.environ.get("RESEARCH_EXTRA_EFFORT", "false").lower() == "true"
            ...
            if uploaded_files:
                logger.info(f"[UPLOAD_TRACE] Processing {len(uploaded_files)} uploaded files")
                try:
                    ...
                    for file_id in uploaded_files:
                        ...
                        if not analysis:
                            logger.info(f"[UPLOAD_TRACE] Triggering analysis for file {file_id}")
                            analysis = await ContentAnalysisService.analyze_file(file_id, "comprehensive")
                        ...
                        if analysis and analysis.status == "completed":
                            ...
                            logger.info(f"[UPLOAD_TRACE] Successfully processed file {file_id}")
                        else:
                            logger.warning(f"[UPLOAD_TRACE] Failed to analyze file {file_id}")
                    
                    # Add analyzed files to state
                    state.analyzed_files = analyzed_files
                    logger.info(f"[UPLOAD_TRACE] Added {len(analyzed_files)} analyzed files to state")
                    
                except Exception as e:
                    logger.error(f"[UPLOAD_TRACE] Error processing uploaded files: {str(e)}")
            ...
            if hasattr(state, 'uploaded_knowledge') and state.uploaded_knowledge:
                ...
                logger.info(f"[UPLOAD_TRACE] State.uploaded_knowledge length: {len(state.uploaded_knowledge)} characters")
                logger.info(f"[UPLOAD_TRACE] State.uploaded_knowledge preview: {state.uploaded_knowledge[:100]}...")
            else:
                logger.info(f"[UPLOAD_TRACE] State created without uploaded_knowledge (value: {getattr(state, 'uploaded_knowledge', 'MISSING_ATTR')})")
            
            if hasattr(state, 'analyzed_files') and state.analyzed_files:
                logger.info(f"[UPLOAD_TRACE] State created with {len(state.analyzed_files)} analyzed files")
                ...
                try:
                    ...
                    if event_type == "node_end":
                        ...
                        if isinstance(output_data, dict):
                            ...
                            if node_name == "deep_search_parallel" and "sources_gathered" in output_data:
                            ...
                            if "web_research_results" in output_data and output_data["web_research_results"]:
                                ...
                                if isinstance(web_results, list) and web_results:
                                    ...
                                    if not hasattr(state, 'sources_gathered') or not state.sources_gathered:
                                        state.sources_gathered = []
                                    
                                    # Process source_citations if needed
                                    if not hasattr(state, 'source_citations') or not state.source_citations:
                                    ...
                                    if hasattr(state, 'sources_gathered') and state.sources_gathered:
                                        sources_gathered.extend(state.sources_gathered)
                                    
                                    if hasattr(state, 'source_citations') and state.source_citations:
                                        ...
                                        for key in source_citations.keys():
                                            if key.isdigit() and int(key) > highest_index:
                                    ...
                                    if sources_gathered:
                                        logger.info(f"[update_state_from_event] Extracted {len(sources_gathered)} sources from web_research_results")
                                        state.sources_gathered = sources_gathered
                                        
                                    if source_citations:
                                        logger.info(f"[update_state_from_event] Created {len(source_citations)} source citations")
                                        state.source_citations = source_citations
                            
                            # Update running summary from report generation nodes
                            if "running_summary" in output_data and isinstance(output_data["running_summary"], str):
                                if len(output_data["running_summary"]) > 100:
                                    state.running_summary = output_data["running_summary"]
                            
                            # Update knowledge_gap from search agent or other nodes
                            if "knowledge_gap" in output_data and output_data["knowledge_gap"]:
                            ...
                            if "research_loop_count" in output_data:
                                state.research_loop_count = output_data["research_loop_count"]
                                
                            # Update any additional state fields that exist in the output
                            for field in ["subtopic_queries", "source_citations", "citations"]:
                    ...
                    # Handle sources_update events
                        ...
                        if sources:
                            state.sources_gathered = sources
                            logger.info(f"[update_state_from_event] Updated sources_gathered with {len(sources)} sources")
                            
                        # Update source_citations if available
                        if source_citations:
                            state.source_citations = source_citations
                            logger.info(f"[update_state_from_event] Updated source_citations with {len(source_citations)} citations")
                        elif citations:
                            state.source_citations = citations
                            logger.info(f"[update_state_from_event] Updated source_citations from citations with {len(citations)} citations")
                    ...
                    try:
                        ...
                        if isinstance(event, dict) and event.get("event_type") == "activity_generated":
                            ...
                            if data.get("related_event_type") == "visualization_generated" and data.get("node_name") == "visualization":
                                ...
                                logger.info(f"Sent visualization activity directly to UI: {data.get('activity', '')[:50]}...")
                                logger.info(f"Event data structure before queuing: {event.get('data', {}).get('enriched_data', {}).keys()}")
                                logger.info(f"Code snippets in event data: {event.get('data', {}).get('enriched_data', {}).get('code_snippets', [])}")
                                return
                        ...
                        if event_type == "node_end" and event_data.get("node_name") == "multi_agents_network":
                            # Extract sources from web_research_results if present
                            output_data = event_data.get("output", {})
                            if isinstance(output_data, dict) and "web_research_results" in output_data:
                                web_results = output_data["web_research_results"]
                                ...
                                if isinstance(web_results, list):
                                    ...
                                    # Update state with extracted sources
                                    if sources_gathered:
                                        logger.info(f"Extracted {len(sources_gathered)} sources from web_research_results")
                                        state.sources_gathered = sources_gathered
                                    
                                    if source_citations:
                                        logger.info(f"Created {len(source_citations)} source citations")
                                        state.source_citations = source_citations
                                    ...
                                    if sources_gathered or source_citations:
                                        ...
                                        logger.info(f"Sent sources update event with {len(sources_gathered)} sources and {len(source_citations)} citations")
                        ...
                        if ActivityManager.should_process_event(event_type, event_data):
                            ...
                            if ActivityManager.is_important_activity(event_type, event_data):
                                # Create a simplified activity event
                                # Data to be passed to ActivityManager.create_activity_event
                                
                                node_input_state = {}
                                if event_type == "node_start":
                                    node_input_state = processed_event.get("data", {}).get("input", {})
                                    if not isinstance(node_input_state, dict): # Ensure it's a dict
                                        logger.warning(f"[_put_event_on_queue] node_input_state for node_start is not a dict: {type(node_input_state)}. Defaulting to empty dict.")
                                        node_input_state = {}
                                
                                # Ensure formatted_sources is a list
                                fs_from_node_input = node_input_state.get("formatted_sources")
                                fs_from_global_state = getattr(state, "formatted_sources", None)
                                ...
                                payload_for_activity_generator = {
                                    "research_topic": node_input_state.get("research_topic", state.research_topic),
                                    "node_name": processed_event.get("data", {}).get("node_name", processed_event.get("name", "")),
                                    "timestamp": processed_event.get("timestamp"),
                                    
                                    "sources": node_input_state.get("sources_gathered", state.sources_gathered),
                                    "citations": node_input_state.get("source_citations", state.source_citations), # Use source_citations for "citations" key too
                                    "source_citations": node_input_state.get("source_citations", state.source_citations),
                                    "web_research_results": node_input_state.get("web_research_results", state.web_research_results),
                                    "formatted_sources": payload_formatted_sources,
                                    "research_loop_count": node_input_state.get("research_loop_count", state.research_loop_count),
                                    ...
                                    **(processed_event.get("data", {})),
                                    ...
                                    "research_topic": node_input_state.get("research_topic", state.research_topic), # Re-assert
                                    "sources": node_input_state.get("sources_gathered", state.sources_gathered), # Re-assert
                                    "citations": node_input_state.get("source_citations", state.source_citations), # Re-assert
                                    "source_citations": node_input_state.get("source_citations", state.source_citations), # Re-assert
                                    "web_research_results": node_input_state.get("web_research_results", state.web_research_results), # Re-assert
                                    "formatted_sources": payload_formatted_sources, # Re-assert
                                    "research_loop_count": node_input_state.get("research_loop_count", state.research_loop_count), # Re-assert
                                    
                                    # Ensure 'input' key itself is correctly passed if it was part of original event_data
                                    # and not just its contents spread out.
                                    "input": node_input_state if event_type == "node_start" else processed_event.get("data", {}).get("input", {})
                                }

                                # Ensure code_snippets are also passed correctly, prioritizing node_input_state
                                node_input_code_snippets = node_input_state.get("code_snippets")
                                event_data_code_snippets = processed_event.get("data", {}).get("code_snippets")
                                event_data_enriched_code_snippets = processed_event.get("data", {}).get("enriched_data", {}).get("code_snippets")

                                if node_input_code_snippets:
                                    payload_for_activity_generator["code_snippets"] = node_input_code_snippets
                                elif event_data_code_snippets:
                                    payload_for_activity_generator["code_snippets"] = event_data_code_snippets
                                elif event_data_enriched_code_snippets:
                                    payload_for_activity_generator["code_snippets"] = event_data_enriched_code_snippets
                                
                                logger.info(f"[_put_event_on_queue] Preparing payload_for_activity_generator for event_type: {event_type}")
                                logger.info(f"[_put_event_on_queue] Using node_input_state: {bool(node_input_state)}")
                                logger.info(f"[_put_event_on_queue] payload_for_activity_generator['sources'] count: {len(payload_for_activity_generator.get('sources', []))}")
                                logger.info(f"[_put_event_on_queue] payload_for_activity_generator['web_research_results'] type: {type(payload_for_activity_generator.get('web_research_results'))}, len: {len(payload_for_activity_generator.get('web_research_results', [])) if isinstance(payload_for_activity_generator.get('web_research_results'), list) else 'N/A'}")
                                logger.info(f"[_put_event_on_queue] payload_for_activity_generator['formatted_sources'] type: {type(payload_for_activity_generator.get('formatted_sources'))}, len: {len(payload_for_activity_generator.get('formatted_sources', [])) if isinstance(payload_for_activity_generator.get('formatted_sources'), list) else 'N/A'}")
                                logger.info(f"[_put_event_on_queue] payload_for_activity_generator keys: {list(payload_for_activity_generator.keys())}")

                                # Create the activity event - this is now async
                                activity_events = await ActivityManager.create_activity_event(
                                    event_type, 
                                    payload_for_activity_generator
                                )
                                ...
                                if activity_events:
                                    ...
                                    for activity_event in activity_events:
                                        logger.info(f"Generated activity for {event_type}: {activity_event.get('data', {}).get('activity', '')}")
                                        # Add source information and code snippets to the event
                                        activity_event['data']['sources'] = state.sources_gathered
                                        activity_event['data']['citations'] = state.source_citations
                                        activity_event['data']['source_citations'] = state.source_citations
                                        
                                        # Ensure code snippets are preserved in enriched_data
                                        if 'enriched_data' not in activity_event['data']:
                                            activity_event['data']['enriched_data'] = {}
                                        if 'code_snippets' not in activity_event['data']['enriched_data']:
                                            activity_event['data']['enriched_data']['code_snippets'] = []
                                        # Preserve any code snippets from the original event
                                        if event_data.get('enriched_data', {}).get('code_snippets'):
                                            activity_event['data']['enriched_data']['code_snippets'].extend(
                                                event_data['enriched_data']['code_snippets']
                                            )
                                        
                                        # Check if this is a visualization activity
                                        is_viz = False
                                        if activity_event.get("data", {}).get("related_event_type") == "visualization_generated":
                                            is_viz = True
                                            logger.info("VISUALIZATION ACTIVITY DETECTED in activity_events list")
                                            images = activity_event.get("data", {}).get("enriched_data", {}).get("images", [])
                                            logger.info(f"Visualization activity has {len(images)} images")
                                            for i, img in enumerate(images):
                                                logger.info(f"  - Image {i+1} has src attribute: {bool(img.get('src'))}")
                                        
                                        await queue.put(activity_event)
                                        logger.info(f"Sent {'visualization ' if is_viz else ''}activity to UI queue: {activity_event.get('data', {}).get('activity', '')[:50]}...")
                        ...
                        if event_type in important_ui_events:
                            ...
                            processed_event['data']['citations'] = state.source_citations
                            processed_event['data']['source_citations'] = state.source_citations
                        ...
                        heartbeat_event = event_buffer.check_heartbeat(processed_event.get("timestamp"))
                        if heartbeat_event:
                            await queue.put(heartbeat_event)
                            
                    except Exception as e:
                        logger.error(f"Failed to put event on queue for {stream_id}: {e}")
                        import traceback
                        logger.error(f"Event queue error traceback: {traceback.format_exc()}")

            try:
                # Stream events directly from LangGraph
                async for raw_event in fresh_graph.astream_events(state, graph_config, version="v2"):
                    ...
                    if event_type == "on_chain_start":
                        ...
                        if raw_event.get("name") == "custom_event":
                            ...
                            event_data = ResearchService._make_json_serializable(custom_event.get("data", {}).get("data", {}))
                            
                            # Pass through the custom event with its type
                            event = {
                                "event_type": custom_event.get("data", {}).get("event_type", "custom_event"),
                                "data": event_data,
                                "timestamp": custom_event.get("data", {}).get("timestamp", timestamp)
                            }
                        # Handle legacy custom_search_sources_found format
                        elif raw_event.get("name") == "custom_event" and "custom_event_type" in input_data:
                            ...
                            event = {
                                ...
                                "data": {
                                    ...
                                    "search_domains": input_data.get("search_domains", []),
                                    "search_query_index": input_data.get("search_query_index", 0)
                                },
                                "timestamp": timestamp
                            }
                        else:
                        ...
                        if "benchmark_result" in output_data and isinstance(output_data["benchmark_result"], dict):
                            ...
                            confidence_level = benchmark_result.get("confidence_level", "")
                            evidence = benchmark_result.get("evidence", "") or benchmark_result.get("supporting_evidence", "")
                            sources = benchmark_result.get("sources", [])
                            reasoning = benchmark_result.get("reasoning", "")
                            limitations = benchmark_result.get("limitations", "")
                            expected_answer = benchmark_result.get("expected_answer", "")
                            ...
                            if confidence_level:
                                final_report_content += f"**Confidence:** {confidence_level}\n\n"
                            
                            if evidence:
                                final_report_content += f"**Evidence:** {evidence}\n\n"
                            
                            if reasoning:
                                final_report_content += f"**Reasoning:** {reasoning}\n\n"
                            ...
                            if limitations and limitations.lower() != 'none':
                                final_report_content += f"**Limitations:** {limitations}\n\n"
                            
                            if expected_answer:
                                final_report_content += f"**Expected Answer:** {expected_answer}\n\n"
                            
                            if is_correct is not None:
                                final_report_content += f"**Correctness:** {'Correct' if is_correct else 'Incorrect'}\n\n"
                            
                            logger.info(f"Found benchmark_result in {raw_event.get('name', 'unknown')}, using enhanced structured answer for final report")
                        ...
                        elif "running_summary" in output_data and isinstance(output_data["running_summary"], str) and len(output_data["running_summary"]) > 200:
                            # This could be part of the final report - store it
                            logger.info(f"Found substantial running_summary in {raw_event.get('name', 'unknown')}, length: {len(output_data['running_summary'])}")
                            state_dict["final_report_content"] = output_data["running_summary"]
                        
                        # Check if this is deep_search_parallel node with search domains info
                        if raw_event.get("name") == "deep_search_parallel" and "search_domains_info" in output_data:
                            search_domains_list = output_data.get("search_domains_info", [])
                            print(f"DEBUG: Found {len(search_domains_list)} search domains sets")
                            ...
                            for domain_set in search_domains_list:
                                ...
                                if domains:
                                    print(f"DEBUG: Emitting search sources event for query: {query}")
                                    event = {
                        ...
                        chunk_content = raw_event.get("data", {}).get("chunk", {}).content
                        if chunk_content:
                            event = {
                        ...
                        if "event_type" in raw_event.get("data", {}):
                            ...
                            custom_event = raw_event.get("data", {})
                            event = {
                                "event_type": custom_event.get("event_type", "tool_event"),
                                "data": ResearchService._make_json_serializable(custom_event.get("data", {})),
                        ...
                        # Handle custom_event container for our updated emit_event function
                            ...
                            event = {
                                "event_type": custom_event.get("event_type", "custom_event"),
                                "data": ResearchService._make_json_serializable(custom_data),
                                "timestamp": custom_event.get("timestamp", timestamp)
                            }
                        else:
                        ...
                        event = {
                            ...
                            "data": ResearchService._make_json_serializable(raw_event.get("data", {})),
                            "timestamp": timestamp
                        }
            ...
            if state_dict["final_report_content"]:
                logger.info(f"Sending research_complete with report content, length: {len(state_dict['final_report_content'])}")
            else:
                logger.warning(f"No final report content found for {stream_id} - sending research_complete without report")
            ...
            if state_dict["final_report_content"]:
                ...
                cleaned_report = event_buffer._clean_content_for_frontend(report_content)
            ...
            try:
                ...
                if complete_event["data"]["report"]:
                    complete_event["data"]["report"] = event_buffer._clean_content_for_frontend(complete_event["data"]["report"])
                ...
                if complete_event["data"]["report"]:
                    report_preview = complete_event["data"]["report"][:200] + "..." if len(complete_event["data"]["report"]) > 200 else complete_event["data"]["report"]
                ...
                if queue:  # Check if queue exists before putting
                    ...
                    logger.info(f"Non-streaming research complete for {stream_id}, no queue to send complete_event.")
                    ...
                    return ResearchResponse(
                        ...
                        research_loop_count=state.research_loop_count if hasattr(state, "research_loop_count") else 0,
                        sources_gathered=state.sources_gathered if hasattr(state, "sources_gathered") else [],
                        web_research_results=state.web_research_results if hasattr(state, "web_research_results") else [],
                        source_citations=state.source_citations if hasattr(state, "source_citations") else {},
                        benchmark_mode=state.benchmark_mode if hasattr(state, "benchmark_mode") else False,
                        benchmark_result=state.benchmark_result if hasattr(state, "benchmark_result") else None,
                        visualizations=state.visualizations if hasattr(state, "visualizations") else [],
                        base64_encoded_images=state.base64_encoded_images if hasattr(state, "base64_encoded_images") else [],
                        visualization_paths=state.visualization_paths if hasattr(state, "visualization_paths") else [],
                        code_snippets=state.code_snippets if hasattr(state, "code_snippets") else [],
                        uploaded_knowledge=uploaded_data_content,
                        analyzed_files=state.analyzed_files if hasattr(state, "analyzed_files") else []
                    )
                else: # streaming but no queue
                    logger.warning(f"Streaming research for {stream_id}, but queue is None. Cannot send complete_event.")
            except Exception as e:
            ...
            if queue:  # Check if queue exists before putting sentinel
                ...
                logger.info(f"Research task for {stream_id} finished processing and sentinel sent.")
            elif not streaming:
                logger.info(f"Non-streaming research for {stream_id}, no queue for sentinel.")
            else: # streaming but no queue
                logger.warning(f"Streaming research for {stream_id}, but queue is None. Cannot send sentinel.")
            ...
            if "Cancelled by cancel scope" in error_message:
                logger.info(f"LangGraph cancel scope error detected. Sending graceful termination event.")
                # Send a graceful completion event rather than an error to prevent frontend retries
                completion_event = {
                    "event_type": "research_complete",
                    "data": {
                        "message": "Research process completed.",
                        "research_topic": state.research_topic if hasattr(state, "research_topic") else query,
                        "report": "The research process completed. Results will be available shortly."
                    },
                    "timestamp": datetime.datetime.now().isoformat()
                }
                if queue: await _put_event_on_queue(completion_event) # Check queue exists
            else:
                ...
                if queue: await _put_event_on_queue(error_event) # Check queue exists
            ...
            logger.error(f"Error during research process for {stream_id}: {e}", exc_info=True)
            error_event = {
                "event_type": "error",
                "data": {"error": str(e), "research_topic": state.research_topic if hasattr(state, "research_topic") else query},
                "timestamp": datetime.datetime.now().isoformat()
            }
            ...
            if not streaming:
             ...
             logger.info(f"Research task for {stream_id} finished processing in finally block.")
             ...
             # For non-streaming requests that reach this point without returning earlier,
             # return a fallback response
             if not streaming:
                 from models.research import ResearchResponse
                 return ResearchResponse(
                     running_summary="Research completed, but no proper report was generated.",
                     research_complete=True,
                     research_loop_count=state.research_loop_count if hasattr(state, "research_loop_count") else 0,
                     sources_gathered=state.sources_gathered if hasattr(state, "sources_gathered") else [],
                     uploaded_knowledge=uploaded_data_content,
    ...
    async def add_queue(stream_id: str, queue: Queue):
        ...
        async with ResearchService._queue_lock:
            if stream_id in ResearchService._active_queues:
                logger.warning(f"Queue already exists for stream {stream_id}. Overwriting.")
    ...
    async def remove_queue(stream_id: str):
        ...
        async with ResearchService._queue_lock:
            if stream_id in ResearchService._active_queues:
                ...
                logger.warning(f"Attempted to remove non-existent queue for stream {stream_id}")

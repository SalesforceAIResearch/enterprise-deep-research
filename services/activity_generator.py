...
class ActivityManager:
    ...
    @staticmethod
    def is_important_activity(event_type: str, event_data: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        elif event_type in ["linkedin_search_results", "academic_search_results", "general_deep_search_results"]:
            return True
    ...
    def _extract_context(event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if event_type == "node_start" and event_data.get("node_name", "").lower() == "reflect_on_report":
        ...
        logger.info(f"Using simplified context extraction. Full event data passed to LLM for interpretation.")
    ...
    def _build_prompt(activity_type: ActivityType, context: Dict[str, Any]) -> str:
            ...
            if subtopic_results:
                ...
                for i, result in enumerate(subtopic_results):
                    if isinstance(result, dict):
                        ...
                        source_counts[tool_used] = source_counts.get(tool_used, 0) + len(sources_list)
                        
                        # Log first subtopic details as an example
                        if i == 0:
                            logger.info(f"Example subtopic: {subtopic.get('name', 'unknown')}")
                            logger.info(f"Example query: {subtopic.get('query', 'unknown')}")
        ...
        context_lines = []
        if node_name:
            context_lines.append(f"The current activity is: {node_name.replace('_', ' ').title()}.")
        
        # Count domains that are actually included in the input data
        prompt_domains_count = 0
        if domains:
            prompt_domains_count = len(domains)
            logger.info(f"Counted {prompt_domains_count} domains directly from input data for prompt.")
            
        # Add domains count to context if there are any
        if prompt_domains_count > 0:
            context_lines.append(f"Processing {prompt_domains_count} websites/sources from input data.")
        
        if domains:
            # Limit displayed domain URLs to avoid overly long prompts
            max_display_domains = 5
            domain_urls_list = [d.get('url', d.get('domain', '')) for d in domains if d.get('url') or d.get('domain')]
        ...
        if sources:
            ...
            filtered_sources = [s for s in sources if not any(d.get('domain') in s for d in domains if d.get('domain'))]
            if filtered_sources:
        ...
        if activity_type == ActivityType.IDENTIFY_GAPS and node_name.lower() == "reflect_on_report":
            # Add Knowledge Gap if available
            if "knowledge_gap" in context:
                context_lines.append(f"Knowledge Gap: {context['knowledge_gap']}")
                logger.info(f"Including knowledge_gap in prompt context: {context['knowledge_gap']}")
                
            # Add Search Query if available
            if "search_query" in context:
                context_lines.append(f"Search Query: {context['search_query']}")
                logger.info(f"Including search_query in prompt context: {context['search_query']}")
...
When describing search activities, mention the specific domains being searched. When describing report generation, mention how many sources are being used, but always in plain, simple language.
"""
    ...
    async def _track_activity(activity_type: str, activity_message: str) -> None:
        ...
        logger.info("Cleared activity cache, image tracking, domain tracking, and batch tracking after 30 minutes of inactivity")

    @staticmethod
    def associate_code_with_visualizations_static(code_snippets, visualization_results, event_id=None):
        """
        ...
        logger.debug(f"{log_prefix}Code snippets count: {len(code_snippets) if code_snippets else 0}, " +
                    f"Visualizations count: {len(visualization_results) if visualization_results else 0}")
        
        if not code_snippets:
            logger.info(f"{log_prefix}No code snippets to process. Returning all visualizations as standalone.")
            return [], visualization_results if visualization_results else []
            
        if not visualization_results:
            logger.info(f"{log_prefix}No visualization results to associate. Returning snippets as-is.")
        ...
        for snippet_idx, snippet in enumerate(code_snippets):
            ...
            if not isinstance(snippet, dict):
                logger.warning(f"{log_prefix}Skipping non-dictionary snippet: {type(snippet)}")
                snippets_with_visuals.append(snippet)
                continue
                
            if 'code' not in snippet:
                logger.warning(f"{log_prefix}Skipping snippet without code: {snippet_id}")
                snippets_with_visuals.append(snippet)
                continue
            ...
            vis_indicators = [
                ...
                'visualize', 'chart', 'plot(', 'figure', 'savefig'
            ]
            ...
            if is_vis_generator:
                logger.debug(f"{log_prefix}Snippet {snippet_id} likely generates visualization. " +
                           f"Score: {vis_score}, Indicators: {matched_indicators}")
            else:
                logger.debug(f"{log_prefix}Snippet {snippet_id} unlikely to generate visualization")
            ...
            for idx, vis_item in enumerate(visualization_results):
                ...
                if idx in processed_vis_indices:
                    logger.debug(f"{log_prefix}Skipping already processed visualization: {vis_id}")
                    continue
                
                description_match = False
                if ('description' in vis_item and 'purpose' in snippet and
                    vis_item['description'] and snippet['purpose'] and
                    any(word in vis_item['description'].lower() for word in snippet['purpose'].lower().split())):
                    description_match = True
                    logger.debug(f"{log_prefix}Found description match between snippet {snippet_id} and vis {vis_id}")
                    
                processed_vis_indices.add(idx)
                association_count += 1
                logger.info(f"{log_prefix}Associating visualization {vis_id} with code snippet {snippet_id}" +
                          (" based on description match" if description_match else ""))
                
                vis_data = {}
                if 'src' in vis_item:
                    vis_data['src'] = vis_item['src']
                    src_info = "base64 data" if vis_item['src'].startswith('data:image') else "file path"
                    src_length = len(vis_item['src']) if vis_item['src'].startswith('data:image') else "N/A"
                    logger.debug(f"{log_prefix}Using src from visualization: {vis_id} ({src_info}, length: {src_length})")
                    ...
                    logger.debug(f"{log_prefix}Created data URI for visualization: {vis_id} " +
                ...
                if 'description' in vis_item:
                    ...
                    logger.debug(f"{log_prefix}Added description to visualization: {vis_id}")
            ...
            if not found_match:
                logger.info(f"{log_prefix}No matching visualization found for snippet {snippet_id}")
                snippets_with_visuals.append(snippet)
        ...
        logger.info(f"{log_prefix}Association complete: {association_count} code snippets associated.")
        logger.info(f"{log_prefix}{len(standalone_visualizations)} visualizations remain standalone.")
                
        return snippets_with_visuals, standalone_visualizations

    @staticmethod
    @retry(stop=stop_after_attempt(2), wait=wait_exponential(multiplier=1, min=1, max=3))
    async def _generate_dynamic_message(activity_type: ActivityType, context: Dict[str, Any]) -> str:
        """
    ...
    async def _generate_dynamic_message(activity_type: ActivityType, context: Dict[str, Any]) -> str:
        ...
        """
        try:
            logger.info(f"===== GENERATING DYNAMIC MESSAGE FOR ACTIVITY TYPE: {activity_type} =====")
            
            # Log key context data available for this activity
            logger.info(f"Context for dynamic message: research_topic='{context.get('research_topic', 'unknown')}'")
            ...
            if search_operations_completed:
                logger.info(f"SEARCH OPERATIONS COMPLETED: {search_operations_completed}")
                ...
                logger.info(f"SEARCH RESULTS STRUCTURE:\n" + 
                ...
                if subtopic_results:
                    ...
                    logger.info(f"EXAMPLE SUBTOPIC: {first_subtopic.get('name', 'unknown')}")
                    logger.info(f"EXAMPLE SUBTOPIC QUERY: {first_subtopic.get('query', 'unknown')}")
                    
                    # Check sources and domains in this result
                    first_search_result = first_result.get('search_result', {})
                    sources_count = len(first_search_result.get('formatted_sources', []))
                    domains = first_search_result.get('domains', [])
                    logger.info(f"EXAMPLE RESULT - Sources: {sources_count}, Domains: {domains}")
            else:
            ...
            if 'event_data' in context:
                logger.info(f"Using simplified context with full event_data, keys: {list(context['event_data'].keys())[:10]}{'...' if len(context['event_data'].keys()) > 10 else ''}")
            ...
            llm = get_llm_client("openai", "o4-mini")
    ...
    @staticmethod
    async def create_activity_event(event_type: str, event_data: Dict[str, Any]) -> Union[Dict[str, Any], List[Dict[str, Any]]]: # Return type hint added
        """
        ...
        try:
            ...
            logger.info(f"===== [EVENT-{event_id}] PROCESSING EVENT: {event_type} =====")
            ...
            logger.info(f"[EVENT-{event_id}] raw_data['sources'] (for sources_from_state): {raw_data.get('sources')}")
            logger.info(f"[EVENT-{event_id}] raw_data['domains'] (for domains_data prio 1): {raw_data.get('domains')}")
            if isinstance(raw_data.get('input'), dict):
                logger.info(f"[EVENT-{event_id}] raw_data['input']['domains'] (for domains_data prio 2): {raw_data['input'].get('domains')}")
            else:
                logger.info(f"[EVENT-{event_id}] raw_data['input'] is not a dict or missing.")

            web_research_results_log = raw_data.get('web_research_results')
            logger.info(f"[EVENT-{event_id}] raw_data['web_research_results'] (for domains_data prio 3): {type(web_research_results_log)} - Len: {len(web_research_results_log) if isinstance(web_research_results_log, list) else 'N/A'}")
            if isinstance(web_research_results_log, list) and web_research_results_log:
                 ...
                 if len(first_item_str) > 500:
                     logger.info(f"[EVENT-{event_id}] First item of web_research_results: {first_item_str[:200]}... [TRUNCATED - full length: {len(first_item_str)}]")
                 else:
                     logger.info(f"[EVENT-{event_id}] First item of web_research_results: {first_item_str}")

            formatted_sources_log = raw_data.get('formatted_sources')
            logger.info(f"[EVENT-{event_id}] raw_data['formatted_sources'] (for domains_data prio 4): {type(formatted_sources_log)} - Len: {len(formatted_sources_log) if isinstance(formatted_sources_log, list) else 'N/A'}")
            if isinstance(formatted_sources_log, list) and formatted_sources_log:
                first_item_str = str(formatted_sources_log[0])
                # Truncate if too long
                if len(first_item_str) > 500:
                    logger.info(f"[EVENT-{event_id}] First item of formatted_sources: {first_item_str[:200]}... [TRUNCATED - full length: {len(first_item_str)}]")
                else:
                    logger.info(f"[EVENT-{event_id}] First item of formatted_sources: {first_item_str}")
            ...
            for path in paths_to_check:
                ...
                if valid_path and current:
                    has_results = True
                    results_path = path
                    logger.info(f"[EVENT-{event_id}] Found visualization results at path: {path}")
                    
                    # Check content type
                    if isinstance(current, list):
                        logger.info(f"[EVENT-{event_id}] Results is a list with {len(current)} items")
                        # Show first item sample
                        if current and len(current) > 0:
                            sample = current[0]
                            if isinstance(sample, dict):
                                logger.info(f"[EVENT-{event_id}] Sample keys: {list(sample.keys())}")
                            else:
                                logger.info(f"[EVENT-{event_id}] Sample type: {type(sample)}")
                    elif isinstance(current, dict):
                        logger.info(f"[EVENT-{event_id}] Results is a dict with keys: {list(current.keys())}")
            
            if not has_results:
                logger.info(f"[EVENT-{event_id}] No visualization results found in this event")
            
            # Extract code snippets from the event data
            code_snippets = []
            logger.info(f"[EVENT-{event_id}] Initializing code_snippets list (currently empty).") # Log initialization

            # --- START MODIFICATION ---
            # Prioritize checking the top-level 'code_snippets' key first
            if isinstance(raw_data, dict) and 'code_snippets' in raw_data and isinstance(raw_data['code_snippets'], list):
                top_level_snippets = raw_data.get('code_snippets', [])
                if top_level_snippets:
                    logger.info(f"[EVENT-{event_id}] Found {len(top_level_snippets)} snippets directly in raw_data['code_snippets'].")
                    code_snippets.extend(top_level_snippets)
                else:
                    logger.info(f"[EVENT-{event_id}] No snippets found directly in raw_data['code_snippets'].")
            # --- END MODIFICATION ---

            if isinstance(raw_data, dict):
                # Check for code snippets in enriched_data (as a fallback or secondary location)
                if 'enriched_data' in raw_data and isinstance(raw_data['enriched_data'], dict):
                    found_in_enriched = raw_data['enriched_data'].get('code_snippets', [])
                    if found_in_enriched:
                         logger.info(f"[EVENT-{event_id}] Found {len(found_in_enriched)} snippets in raw_data['enriched_data'].")
                         code_snippets.extend(found_in_enriched) # Extend, don't overwrite
                ...
                def extract_nested_code_snippets(data, path=""): # Add path tracking
                    ...
                    if isinstance(data, dict):
                        ...
                        if 'code_snippets' in data and isinstance(data['code_snippets'], list):
                            ...
                            if found_nested:
                                logger.info(f"[EVENT-{event_id}] Found {len(found_nested)} nested snippets at path: {current_path}.code_snippets")
                        ...
                        for key, value in data.items():
                            ...
                            if key == 'code_snippets' and path == "":
                                continue
                            snippets.extend(extract_nested_code_snippets(value, f"{current_path}.{key}"))
                    elif isinstance(data, (list, tuple)):
                        for i, item in enumerate(data):
                            snippets.extend(extract_nested_code_snippets(item, f"{current_path}[{i}]"))
                    return snippets
                ...
                if not code_snippets: # <-- Check if snippets were already found
                    logger.info(f"[EVENT-{event_id}] No top-level snippets found, searching nested 'input' and 'results'...")
                    if 'input' in raw_data:
                        logger.info(f"[EVENT-{event_id}] Searching for snippets within raw_data['input']...")
                        input_snippets = extract_nested_code_snippets(raw_data['input'], "input")
                        if input_snippets:
                             logger.info(f"[EVENT-{event_id}] Found {len(input_snippets)} snippets within raw_data['input'].")
                    ...
                    if 'results' in raw_data:
                        logger.info(f"[EVENT-{event_id}] Searching for snippets within raw_data['results']...")
                        results_snippets = extract_nested_code_snippets(raw_data['results'], "results")
                        if results_snippets:
                             logger.info(f"[EVENT-{event_id}] Found {len(results_snippets)} snippets within raw_data['results'].")
                             code_snippets.extend(results_snippets)
                        # else: # Reduce log noise
                        #      logger.info(f"[EVENT-{event_id}] No snippets found within raw_data['results'].")
                else:
                     logger.info(f"[EVENT-{event_id}] Skipping nested search as top-level snippets were found.")

            else:
                 logger.warning(f"[EVENT-{event_id}] raw_data is not a dict, cannot search for snippets.")
                ...
                for snippet in code_snippets:
                    if isinstance(snippet, dict):
                        ...
                        if code_content:
                         ...
                         logger.warning(f"[EVENT-{event_id}] Skipping non-dict item during deduplication: {type(snippet)}")

                deduplicated_code_snippets = list(unique_snippets_map.values())
                logger.info(f"[EVENT-{event_id}] Found {len(code_snippets)} total code snippets, {len(deduplicated_code_snippets)} unique after deduplication.")

                # Ensure enriched_data exists and add snippets
                if isinstance(raw_data, dict):
                    logger.info(f"[EVENT-{event_id}] Raw data is a dict. Proceeding to add snippets.")
                    if 'enriched_data' not in raw_data or not isinstance(raw_data.get('enriched_data'), dict):
                        logger.info(f"[EVENT-{event_id}] Initializing empty enriched_data.")
                    ...
                    existing_snippet_list = raw_data['enriched_data'].get('code_snippets', [])
                    # Use a simpler deduplication based on the unique map we already created
                    final_snippets_map = {hash(s.get('code','')): s for s in existing_snippet_list if isinstance(s, dict) and s.get('code')}
                    added_count = 0
                    ...
                    logger.info(f"[EVENT-{event_id}] Added {added_count} new unique snippets to the list.")
                    raw_data['enriched_data']['code_snippets'] = final_combined_snippets
                    logger.info(f"[EVENT-{event_id}] Successfully updated enriched_data. Final total code snippets: {len(final_combined_snippets)}")
                else:
                    logger.warning(f"[EVENT-{event_id}] Raw data is not a dict ({type(raw_data)}), cannot add snippets.")
            else:
                 logger.info(f"[EVENT-{event_id}] No code snippets found after searching raw_data.")


            # Regular processing continues...
            logger.info(f"===== [EVENT-{event_id}] CREATING ACTIVITY EVENT FOR: {event_type} =====")
            logger.info(f"[EVENT-{event_id}] Event data keys: {list(event_data.keys())}")
            ...
            if not research_topic and isinstance(raw_data.get('input'), dict):
                research_topic = raw_data['input'].get('research_topic', 'unknown_topic')
            
            # Extract source information from raw_data
            sources_from_state = raw_data.get('sources', []) # This is likely state.sources_gathered
            ...
            if isinstance(raw_data.get('domains'), list):
                ...
                logger.info(f"[EVENT-{event_id}] Found {len(potential_domains)} domains in raw_data['domains']")
                for d_item in potential_domains:
                    if isinstance(d_item, dict) and 'url' in d_item:
                        url = d_item['url']
                        if url and url not in seen_domain_urls:
                            domains_data.append({'url': url, 'title': d_item.get('title', extract_domain(url))})
                        ...
                        url = domain_str_cleaned if domain_str_cleaned.startswith('http') else f"https://{domain_str_cleaned}"
                        if url not in seen_domain_urls:
                            domains_data.append({'url': url, 'title': extract_domain(domain_str_cleaned)})
                            seen_domain_urls.add(url)

            # Priority 2: 'input.domains'
            if not domains_data and isinstance(raw_data.get('input'), dict) and isinstance(raw_data['input'].get('domains'), list):
                potential_domains = raw_data['input']['domains']
                logger.info(f"[EVENT-{event_id}] Found {len(potential_domains)} domains in raw_data['input']['domains']")
                for d_item in potential_domains:
                    if isinstance(d_item, dict) and 'url' in d_item:
                        url = d_item['url']
                        if url and url not in seen_domain_urls:
            ...
            if not domains_data and isinstance(web_results_for_domains, list):
                logger.info(f"[EVENT-{event_id}] Attempting to extract domains from raw_data['web_research_results'] ({len(web_results_for_domains)} items)")
                for result_item in web_results_for_domains:
                    if isinstance(result_item, dict) and isinstance(result_item.get('domains'), list):
                        for domain_str in result_item['domains']:
                            if isinstance(domain_str, str) and domain_str.strip():
                                ...
                                if url not in seen_domain_urls:
                                    ...
                                    seen_domain_urls.add(url)
                if domains_data:
                    logger.info(f"[EVENT-{event_id}] Extracted {len(domains_data)} domains from raw_data['web_research_results']")
            ...
            if not domains_data and isinstance(formatted_sources_for_domains, list):
                logger.info(f"[EVENT-{event_id}] Attempting to extract domains from raw_data['formatted_sources']")
                for source_str_entry in formatted_sources_for_domains:
                    if isinstance(source_str_entry, str) and ' : ' in source_str_entry:
                        ...
                        if len(parts) >= 2:
                            url = parts[1].strip()
                            if url and url not in seen_domain_urls: # Ensure URL is not empty
                                title = extract_domain(url)
                                domains_data.append({'url': url, 'title': title})
                                seen_domain_urls.add(url)
                if domains_data:
                     logger.info(f"[EVENT-{event_id}] Extracted {len(domains_data)} domains from raw_data['formatted_sources']")
            ...
            if is_new_loop:
                logger.info(f"RESET EVENT: New loop {loop_id} (was {ActivityManager._current_research_loop})")
            ...
            if domains_data:
                logger.info(f"[EVENT-{event_id}] Processing {len(domains_data)} domains for global deduplication")
                
                # Filter out domains that have already been sent globally
                new_domains_for_event = []
                for domain in domains_data:
                    domain_url = domain.get('url', '')
                    if domain_url and domain_url not in ActivityManager._sent_domain_urls:
                        ...
                        logger.info(f"[EVENT-{event_id}] Skipping already sent domain: {domain_url}")
                
                domains_data_for_event = new_domains_for_event
                logger.info(f"[EVENT-{event_id}] After global deduplication: {len(domains_data_for_event)} domains to show")
                logger.info(f"[EVENT-{event_id}] Total domains tracked globally: {len(ActivityManager._sent_domain_urls)}")
            else:
            ...
            if node_name and "multi_agents_network" not in node_name.lower():
                ...
                logger.info(f"Clearing domains for non-multi_agents_network node: {node_name}")
            
            # Also clear domains for certain event types that should never show domains
            elif event_type in ["visualization_generated", "knowledge_gap_identified", "knowledge_gap"]:
                should_clear_domains = True
                logger.info(f"Clearing domains for event type {event_type} that should never show domains")
            ...
            # Special handling for activity_generated events - check the related_event_type and node_name
                ...
                if activity_node_name and "multi_agents_network" not in activity_node_name.lower():
                    should_clear_domains = True
                    logger.info(f"Clearing domains for activity_generated event from non-multi_agents_network node: {activity_node_name}")
                elif related_event_type and "multi_agents_network" not in related_event_type.lower():
                    should_clear_domains = True
                    logger.info(f"Clearing domains for activity_generated event with non-multi_agents_network related_event_type: {related_event_type}")
            
            if should_clear_domains:
                domains_data_for_event = []
                logger.info(f"[EVENT-{event_id}] Domains cleared - web links only allowed in multi_agents_network stage")
            else:
                logger.info(f"[EVENT-{event_id}] Domains allowed for multi_agents_network stage")
            ...
            if isinstance(viz_results_raw, dict) and 'results' in viz_results_raw: # Handle nested structure
            ...
            if not visualization_results and isinstance(raw_data.get('search_result'), dict):
            ...
            if not visualization_results and isinstance(raw_data.get('code_data'), dict):
            ...
            if not visualization_results:
                ...
                if isinstance(viz_results_raw, dict) and 'results' in viz_results_raw: # Handle potential nesting here too
                    visualization_results = viz_results_raw.get('results', [])
                elif isinstance(viz_results_raw, list):
                    visualization_results = viz_results_raw
            ...
            if not visualization_results and 'input' in raw_data:
                ...
                if isinstance(input_data, dict):
                    ...
                    for field in ['results', 'visualizations', 'visualization_results', 'base64_encoded_images']:
            ...
            if isinstance(visualization_results, list) and visualization_results:
                logger.info(f"Found visualization results list. Processing {len(visualization_results)} items for images.")
                ...
                for item in visualization_results:
                    ...
                    if isinstance(item, dict):
                        item_sig = f"{item.get('filepath', '')}_{item.get('description', '')}"
                ...
                if potential_duplicates > 0:
                    logger.warning(f"Found {potential_duplicates} potential duplicate images in visualization_results!")
                ...
                for item in visualization_results:
                    ...
                    if isinstance(item, dict):
                        ...
                        if not img_src and img_data_b64: # Construct src from base64 data if src missing
                            img_src = f"data:image/{img_format};base64,{img_data_b64}"
                            logger.info(f"Constructed 'src' from 'data' field for image: {description or filepath} (base64 length: {len(img_data_b64) if img_data_b64 else 0})")

                    elif isinstance(item, str) and os.path.exists(item): # Handle direct file path string
                        filepath = item
                        description = os.path.basename(filepath) # Use filename as description fallback

                    # Now, prioritize src, then filepath encoding
                    if img_src:
                        # Log image info without exposing base64 data
                        src_info = "base64 data" if img_src.startswith('data:image') else "file path"
                        src_length = len(img_src) if img_src.startswith('data:image') else "N/A"
                        images.append({'src': img_src, 'description': description, 'format': img_format})
                        logger.info(f"Using existing/constructed 'src' for image: {description or filepath} ({src_info}, length: {src_length})")
                        ...
                        try:
                            ...
                            images.append({'src': img_src, 'description': description, 'format': img_format})
                            logger.info(f"Successfully encoded image from filepath: {filepath} (base64 length: {len(b64)})")
                        except Exception as img_err:
                            logger.error(f"Error encoding image {filepath} from event data: {img_err}")
                    else:
                        # Log item info without potentially large data
                        item_info = str(item)
                        if len(item_info) > 200:
                            item_info = item_info[:200] + f"... [TRUNCATED - full length: {len(str(item))}]"
                        logger.warning(f"Could not process visualization item: {item_info}")
            else:
                logger.info(f"No valid visualization results found after checking multiple locations in event data for event {event_type}.")


            # Generate the main activity text
            activity_text = await ActivityManager._generate_activity_text(event_type, event_data)
                ...
                for source_str_entry in sources_from_state:
                    if isinstance(source_str_entry, str) and ' : ' in source_str_entry:
                        ...
                        if len(parts) == 2:
                            ...
                            formatted_sources_for_ui.append({'title': title, 'url': url})
                    elif isinstance(source_str_entry, dict) and 'title' in source_str_entry and 'url' in source_str_entry:
                        formatted_sources_for_ui.append(source_str_entry)

            logger.info(f"[EVENT-{event_id}] Final formatted_sources_for_ui count: {len(formatted_sources_for_ui)}")
            if formatted_sources_for_ui:
                ...
                if isinstance(sample_source, dict):
                    ...
                    sample_str = str(sample_source)[:100] + ("..." if len(str(sample_source)) > 100 else "")
                logger.info(f"[EVENT-{event_id}] Sample formatted_sources_for_ui[0]: {sample_str}")
            logger.info(f"[EVENT-{event_id}] Final domains_data_for_event count: {len(domains_data_for_event)}")
            if domains_data_for_event:
                ...
                if isinstance(sample_domain, dict):
                    ...
                    sample_str = str(sample_domain)[:100] + ("..." if len(str(sample_domain)) > 100 else "")
                logger.info(f"[EVENT-{event_id}] Sample domains_data_for_event[0]: {sample_str}")
            ...
            if images and len(images) > 0:
                logger.info(f"[EVENT-{event_id}] Processing {len(images)} raw visualization images")
                ...
                for idx, img in enumerate(images):
                    ...
                    logger.info(f"[EVENT-{event_id}] Image {idx+1}/{len(images)} - Identifier: {filename}")
                    is_already_sent_globally = filename in ActivityManager._sent_image_filenames
                    logger.info(f"[EVENT-{event_id}] Image {idx+1}/{len(images)} - Already sent globally? {is_already_sent_globally}")

                    # Collect images that haven't been sent globally yet
                    if not is_already_sent_globally:
                        new_images_to_send.append((img, filename)) # Store tuple (image_data, identifier)
                    else:
                         logger.info(f"[EVENT-{event_id}] Skipping globally tracked image: {filename}")

                # Log the current global tracking state
                logger.info(f"[EVENT-{event_id}] Globally tracking {len(ActivityManager._sent_image_filenames)} images before adding new ones")

                # Step 2: If no globally new images, we're done with visualizations for this event
                if not new_images_to_send:
                    logger.info(f"[EVENT-{event_id}] All images in this batch were already sent globally. No visualization events to create.")
                    # Return the main activity event without visualization events
                    return activity_event 
                
                logger.info(f"[EVENT-{event_id}] Identified {len(new_images_to_send)} potentially new images (not sent globally yet)")
                ...
                import hashlib
                new_image_identifiers_sorted = sorted([item[1] for item in new_images_to_send]) # Sort identifiers for consistent hashing
                batch_signature = hashlib.md5(str(new_image_identifiers_sorted).encode()).hexdigest()
                ...
                if batch_signature in ActivityManager._processed_batches:
                    logger.warning(f"[EVENT-{event_id}] DUPLICATE BATCH of new images detected! Signature: {batch_signature[:10]}...")
                    logger.warning(f"[EVENT-{event_id}] This exact set of {len(new_images_to_send)} new images was processed recently. Skipping.")
                    ...
                    logger.info(f"[EVENT-{event_id}] New batch of {len(new_images_to_send)} unsent images with signature: {batch_signature[:10]}...")

                # Step 4: Create visualization events for the new images, up to the limit
                display_limit = 5 # Use the limit you set previously
                images_to_display = new_images_to_send[:min(len(new_images_to_send), display_limit)]
                
                logger.info(f"[EVENT-{event_id}] Preparing to create events for {len(images_to_display)} new images (limit: {display_limit})")
                ...
                for img_data, filename in images_to_display:
                    ...
                    if filename not in ActivityManager._sent_image_filenames:
                         ...
                         logger.info(f"[EVENT-{event_id}] Added {filename} to global tracking.")
                    ...
                    logger.info(f"[EVENT-{event_id}] Created visualization event {len(visualization_events)} for image {filename}")

                # Log final global tracking state after processing this batch
                logger.info(f"[EVENT-{event_id}] Globally tracking {len(ActivityManager._sent_image_filenames)} images after processing this batch.")
            ...
            if visualization_events:
                ...
                logger.info(f"[EVENT-{event_id}] Returning {len(final_return_value)} events: 1 main activity (images removed) + {len(visualization_events)} visualization events.")
            else:
                # Return only the main activity event (which now includes images if found)
                final_return_value = activity_event
                logger.info(f"[EVENT-{event_id}] Returning 1 main activity event. Images included: {len(activity_event['data']['enriched_data']['images'])}")
    ...
    @staticmethod
    async def _generate_activity_text(event_type: str, event_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if event_type == "node_start" and "node_name" in event_data:
                ...
                if "generate_report" in node_name or "report_generator" in node_name:
                    ...
                    logger.info(f"No specific mapping for node: {node_name}, using NODE_START")
            elif event_type == "search_sources_found":
                activity_type = ActivityType.GENERAL_DEEP_SEARCH
            ...
            message = await ActivityManager._generate_dynamic_message(activity_type, context)
    ...
    @staticmethod
    def should_process_event(event_type: str, event_data: Optional[Dict[str, Any]] = None) -> bool:
        """
    ...
    def reset_image_tracking() -> None:
        ...
        if hasattr(ActivityManager, "_processed_batches"):
            ActivityManager._processed_batches.clear()
        logger.info("Reset image and domain tracking - previously sent images and web links can be sent again") 
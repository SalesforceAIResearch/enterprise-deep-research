...
class ResearchTodoManager:
    ...
    async def create_initial_plan(self, initial_query: str, research_context: str = ""):
        ...
        try:
            ...
            llm = await get_async_llm_client("openai", "gpt-4o-mini")
...
Create 3-5 initial research tasks that break down this query into actionable research steps. Return a JSON array where each task has:
- "description": Clear, actionable research task
- "priority": 1-10 (higher = more important)
- "type": "research" (all initial tasks are research tasks)
...
[
  {{"description": "Research Silvio Savarese's academic background and current position", "priority": 9, "type": "research"}},
  {{"description": "Investigate his key research contributions and publications", "priority": 8, "type": "research"}},
  {{"description": "Explore his industry experience and leadership roles", "priority": 7, "type": "research"}},
  {{"description": "Analyze his impact on computer vision and AI fields", "priority": 6, "type": "research"}}
]
...
"""
            ...
            try:
                ...
                initial_tasks = json.loads(response_text)
                if not isinstance(initial_tasks, list):
                    initial_tasks = [initial_tasks]

            except json.JSONDecodeError:
                logger.warning(
                    f"[STEERING] Failed to parse initial plan: {response.content}"
                )
            ...
            for i, task_data in enumerate(initial_tasks):
                ...
                task = SteeringTask(
                    ...
                    priority=min(10, max(1, task_data.get("priority", 5))),
                    created_from_message=f"Initial query: {initial_query}",
                    search_queries=[],
                )

                self.tasks[task_id] = task
                logger.info(f"[STEERING] Created initial task: {task.description}")
    ...
    async def _process_pending_messages(self):
        ...
        if len(self.pending_messages) > 1:
            ...
            for task in tasks:
                ...
                logger.info(f"[STEERING] Created task: {task.description}")
...
"""
    ...
    async def prepare_for_next_loop(
        ...
        if self.pending_messages:
            ...
            logger.info(
                f"[STEERING] Messages processed, todo.md updated to version {self.todo_version}"
            )
...
"""
    ...
    async def _message_to_tasks(self, message: str) -> List[SteeringTask]:
        ...
        try:
            ...
            # Set max_tokens to avoid the error
            if hasattr(llm, "max_tokens"):
                llm.max_tokens = 500
...
"""
    ...
    def get_pending_tasks(self) -> List[SteeringTask]:
    ...
    def mark_task_completed(self, task_id: str):
        ...
        if task_id in self.tasks:
            ...
            logger.info(f"[STEERING] Completed task: {self.tasks[task_id].description}")

    def mark_task_cancelled(self, task_id: str):
        """Mark task as cancelled"""
        if task_id in self.tasks:
            self.tasks[task_id].status = TaskStatus.CANCELLED
            logger.info(f"[STEERING] Cancelled task: {self.tasks[task_id].description}")

    def mark_task_in_progress(self, task_id: str):
        """Mark a task as in progress"""
        if task_id in self.tasks:
            self.tasks[task_id].status = TaskStatus.IN_PROGRESS
            logger.info(f"[STEERING] Task started: {self.tasks[task_id].description}")

    def create_task(
        self, description: str, priority: int = 5, search_queries: List[str] = None
    ):
        ...
        self.tasks[task_id] = task
        logger.info(f"[STEERING] Created task: {description}")
        return task_id
    ...
    def get_todo_md(self) -> str:
        ...
        if pending_tasks:
            md_content += "## 🎯 Active Steering Instructions\n\n"
            pending_tasks.sort(key=lambda t: t.priority, reverse=True)
            for task in pending_tasks:
                md_content += f"- [ ] **P{task.priority}** {task.description}\n"
        ...
        if active_tasks:
            ...
            for task in active_tasks:
                md_content += f"- [x] **P{task.priority}** {task.description}\n"
                md_content += f'  - *From user:* "{task.created_from_message}"\n\n'
        ...
        if completed_tasks:
            ...
            for task in completed_tasks[-5:]:  # Last 5
                md_content += f"- [x] {task.description}\n"
        ...
        if cancelled_tasks:
            ...
            for task in cancelled_tasks[-3:]:  # Last 3
